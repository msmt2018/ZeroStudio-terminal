...
import android.zero.studio.terminal.app.event.SystemEventReceiver;
import android.zero.studio.terminal.app.terminal.TermuxTerminalSessionActivityClient;
import android.zero.studio.terminal.app.terminal.TermuxTerminalSessionServiceClient;
import android.zero.studio.terminal.shared.termux.plugins.TermuxPluginUtils;
import android.zero.studio.terminal.shared.data.IntentUtils;
import android.zero.studio.terminal.shared.net.uri.UriUtils;
import android.zero.studio.terminal.shared.errors.Errno;
import android.zero.studio.terminal.shared.shell.ShellUtils;
import android.zero.studio.terminal.shared.shell.command.runner.app.AppShell;
import android.zero.studio.terminal.shared.termux.settings.properties.TermuxAppSharedProperties;
import android.zero.studio.terminal.shared.termux.shell.command.environment.TermuxShellEnvironment;
import android.zero.studio.terminal.shared.termux.shell.TermuxShellUtils;
import android.zero.studio.terminal.shared.termux.TermuxConstants;
import android.zero.studio.terminal.shared.termux.TermuxConstants.TERMUX_APP.TERMUX_ACTIVITY;
import android.zero.studio.terminal.shared.termux.TermuxConstants.TERMUX_APP.TERMUX_SERVICE;
import android.zero.studio.terminal.shared.termux.settings.preferences.TermuxAppSharedPreferences;
import android.zero.studio.terminal.shared.termux.shell.TermuxShellManager;
...
import android.zero.studio.terminal.shared.logger.Logger;
import android.zero.studio.terminal.shared.notification.NotificationUtils;
import android.zero.studio.terminal.shared.android.PermissionUtils;
import android.zero.studio.terminal.shared.data.DataUtils;
...
import android.zero.studio.terminal.shared.shell.command.ExecutionCommand.ShellCreateMode;
import android.zero.studio.terminal.terminal.TerminalEmulator;
    ...
    private synchronized void killAllTermuxExecutionCommands() {
        ...
        for (int i = 0; i < pendingPluginExecutionCommands.size(); i++) {
            ...
            if (!executionCommand.shouldNotProcessResults() && executionCommand.isPluginExecutionCommandWithPendingResult()) {
                if (executionCommand.setStateFailed(Errno.ERRNO_CANCELLED.getCode(), this.getString(android.zero.studio.terminal.shared.R.string.error_execution_cancelled))) {
